...
const createPopupComments = (comments, userEmoji, deletingcomment, isDisabled, isDeleting, isSaving) => {
  return (
    `<section class="film-details__comments-wrap">
    <h3 class="film-details__comments-title">Comments <span class="film-details__comments-count">${comments.length}</span></h3>

    <ul class="film-details__comments-list">
      ${createComments(comments, deletingcomment, isDisabled, isDeleting)}
    </ul>
...
export default class PopupComments extends AbstractView {
  ...
  getTemplate() {
    return createPopupComments(this._comments, this._userEmoji, this._deletingComment, this._isDisabled, this._isDeleting, this._isSaving);
  }
}
